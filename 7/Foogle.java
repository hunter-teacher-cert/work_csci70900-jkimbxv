...
public class Foogle{
  ...
  //linear search, returns first index of target in ArrayList
  ...
  //returns an ArrayList of size numItems filled with random integers within range of lo (inclusive) to hi (exclusive)
  ...
  public static ArrayList getRandIntsDynamicRange(int numItems, int lo, int hi){
    ArrayList retArr = new ArrayList();

    // if numItems specified is < 1, return empty ArrayList
    if ( numItems<1 )
      return retArr;

    //add a random int from 0(inclusive) to lo (exclusive) in 0th index
    retArr.add(
               (int)(lo * Math.random())
               );

    //add random ints with bounds [prevElement, hi)
